...
const float RampRateMax = 0.02; // Adjust this value to control the ramp-up speed
float TargetVMotA = 0;
float TargetVMotB = 0;
float TargetVMotC = 0;
float CurrentVMotA = 0;
float CurrentVMotB = 0;
float CurrentVMotC = 0;
...
void loop()
{
    ...
    // Calculate the target motor speeds
    TargetVMotA = (-0.5 * Vx) + (Sqrt3Demi * Vy) + (DCentreRoue * Om);
    TargetVMotB = (-0.5 * Vx) - (Sqrt3Demi * Vy) + (DCentreRoue * Om);
    TargetVMotC = Vx + (DCentreRoue * Om);

    // Gradually ramp-up the motor speeds
    CurrentVMotA += constrain(TargetVMotA - CurrentVMotA, -RampRateMax, RampRateMax);
    CurrentVMotB += constrain(TargetVMotB - CurrentVMotB, -RampRateMax, RampRateMax);
    CurrentVMotC += constrain(TargetVMotC - CurrentVMotC, -RampRateMax, RampRateMax);

    // Find the highest absolute value among the current motor speeds
    Avant = max(max(abs(CurrentVMotA), abs(CurrentVMotB)), abs(CurrentVMotC));
    ...
    {
        FrequenceA = mapFloat(CurrentVMotA, -Avant, Avant, -Apres, Apres) * throttle;
        FrequenceB = mapFloat(CurrentVMotB, -Avant, Avant, -Apres, Apres) * throttle;
        FrequenceC = mapFloat(CurrentVMotC, -Avant, Avant, -Apres, Apres) * throttle;
    }
    // Controle des freins
    ...
    digitalWrite(dir_pin_A, CurrentVMotA >= 0 ? HIGH : LOW);
    digitalWrite(dir_pin_B, CurrentVMotB >= 0 ? HIGH : LOW);
    digitalWrite(dir_pin_C, CurrentVMotC >= 0 ? HIGH : LOW);
