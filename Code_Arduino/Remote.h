...
const char *ssid = "FreeboxAspirateur";
// A modifier avec le mot de passe de votre réseau
const char *password = "gidobo34";
...
void handleWebSocketMessage(void *arg, uint8_t *data, size_t len)
{
  ...
  if (info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT)
  {
    ...
    if (json["joystick_left_x"] || json["joystick_left_y"])
    {
    ...
    // On récupère les valeurs de notre joystick du milieu, on les stockes dans les variables globales joy_mid_x et joy_mid_y (accessibles de partout dans notre programme).
    if (json["joystick_3_x"] || json["joystick_3_y"])
    {
      joystick_3_x = json["joystick_3_x"];
      joystick_3_y = json["joystick_3_y"];

      // On affiche leurs valeurs
      /*Serial.print("joystick_3_x : ");
      Serial.println(joystick_3_x);*/
      Serial.print("joystick_3_y : ");
      Serial.println(joystick_3_y);
    }
  }
}
